...
const createOrderAndInitializePayment = async (req, res) => {
    try {
        const { shippingInfo, items, totalAmount, sessionId, paymentMethod = "card" } = req.body;
        ...
        for (const item of items) {
            const product = await Product.findById(item.productId).select("price stock title seller");
            if (!product) {
            ...
            if (product.price !== item.price) hasPriceChanges = true;
        ...
        if (paymentMethod === "pay_on_delivery") {
            ...
            if (userId) await Cart.findOneAndDelete({ userId });
            else await Cart.findOneAndDelete({ sessionId });
        ...
        const flwRes = await initializeTransaction(orderTotal, userEmail, order._id, origin);

        order.flutterwaveReference = flwRes.data.tx_ref;
        order.flutterwaveAuthorizationUrl = flwRes.data.link;
        await order.save();
        ...
        res.status(201).json({
            ...
            orderId: order._id,
            authorizationUrl: flwRes.data.link,
            reference: flwRes.data.tx_ref,
            paymentRequired: true,
        });
    } catch (error) {
        console.error("Create order error:", error);
    ...
    try {
        ...
        if (!transaction_id || !orderId) {
            return res.status(400).json({
                ...
                received: { transaction_id, orderId },
            });
        }
...
const confirmOrderDelivery = async (req, res) => {
    try {
        ...
        const order = await Order.findById(orderId).populate("items.productId", "seller");
        ...
        order.items.forEach((item) => {
            const sellerId = item.productId.seller.toString();
            if (!sellerPayments[sellerId]) sellerPayments[sellerId] = 0;
            sellerPayments[sellerId] += item.price * item.quantity;
        });
        ...
        for (const [sellerId, amount] of Object.entries(sellerPayments)) {
            try {
                const transferCode = await transferToSeller(order, sellerId, amount, orderId);
                paymentResults.push({ sellerId, success: true, amount, transferCode, mode: isTestMode ? "test" : "live" });
            } catch (error) {
                console.error(`Payment failed for seller ${sellerId}:`, error.message);
                paymentResults.push({ sellerId, success: false, amount, error: error.message, mode: isTestMode ? "test" : "live" });
            }
        }
        ...
        if (allSuccessful) {
            ...
            res.status(200).json({
                message: isTestMode ? "Order confirmed! (Test Mode - Payments Simulated)" : "Order confirmed and payments processed",
                payments: paymentResults,
                mode: isTestMode ? "test" : "live",
            });
        } else {
            const failedCount = paymentResults.filter((p) => !p.success).length;
            res.status(207).json({ message: `${failedCount} payment(s) failed`, payments: paymentResults, mode: isTestMode ? "test" : "live" });
        }
    } catch (error) {
        console.error("Confirm delivery error:", error);
...
const getOrders = async (req, res) => {
    try {
        ...
        if (role === "user") {
            orders = await Order.find({ userId }).populate("items.productId", "title images price").sort({ createdAt: -1 });
        } else if (role === "seller") {
            orders = await Order.find({ "items.sellerId": userId }).populate("items.productId", "title images price").sort({ createdAt: -1 });
        } else {
            return res.status(401).json({ message: "Authentication required" });
        }

        res.status(200).json({ success: true, orders, message: "Orders retrieved successfully" });
    } catch (error) {
        console.error("Get orders error:", error);
        handleError(res, error);
    }
    ...
    try {
        ...
        if (role === "seller") {
            const hasSellerItems = order.items.some(
                (item) => item.productId?.seller?.toString() === userId.toString()
            );
            if (!hasSellerItems) return res.status(403).json({ message: "Not authorized to view this order" });
        }

        res.status(200).json({ success: true, order, message: "Order details retrieved successfully" });
    } catch (error) {
        console.error("Get order details error:", error);
        handleError(res, error);
    }
